...
# Try to find Placement Status automatically
target_candidates = [
    "Placement Status",
    "PlacementStatus",
    "placement status",
...
for column in data.columns:
    if column.strip().lower().replace("_", " ") in [
        x.lower().replace("_", " ") for x in target_candidates
    ]:
        TARGET = column
        break


# If target is not found, stop with a useful message
if TARGET is None:
